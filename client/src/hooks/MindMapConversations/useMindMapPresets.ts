...
import useNavigateToMindMapConvo from '~/hooks/useNavigateToMindMapConvo';
import { cleanupMindMapPreset, getEndpointField } from '~/utils';
import useDefaultMindMapConvo from '~/hooks/useDefaultMindMapConvo';
...
export default function useMindMapPresets(
  id = 0,
  ...
  const { mindMapPreset, mindMapConversation, newMindMapConversation, setMindMapPreset } =
  ...
  useEffect(() => {
    ...
    if (!mindMapConversation?.conversationId || mindMapConversation.conversationId === 'new') {
      newMindMapConversation({ preset: defaultPreset });
    }
    hasLoaded.current = true;
  ...
  const updatePreset = useUpdatePresetMutation({
    onSuccess: (data, preset) => {
      ...
      if (data.defaultPreset && data.presetId !== _defaultPreset?.presetId) {
        ...
        newMindMapConversation({ preset: data });
  ...
  const { navigateToMindMapConvo } = useNavigateToMindMapConvo();
  const getDefaultMindMapConversation = useDefaultMindMapConvo();
  ...
  const onFileSelected = (jsonData: Record<string, unknown>) => {
    const jsonPreset = { ...cleanupMindMapPreset({ mindMapPreset: jsonData }), presetId: null };
  ...
  const onSelectPreset = (newPreset: TPreset) => {
    ...
    if (
      ...
      const currentConvo = getDefaultMindMapConversation({
        /* target endpointType is necessary to avoid endpoint mixing */
        conversation: { ...(mindMapConversation ?? {}), endpointType },
        preset: { ...newPreset, endpointType },
      });

      /* We don't reset the latest message, only when changing settings mid-converstion */
      navigateToMindMapConvo(currentConvo, false);
      return;
    }

    newMindMapConversation({ preset: newPreset });
  };

  const onChangePreset = (preset: TPreset) => {
    setMindMapPreset(preset);
  ...
  const submitPreset = () => {
    ...
    updatePreset.mutate(cleanupMindMapPreset({ mindMapPreset }));
  };
  ...
  const exportPreset = () => {
    ...
    const fileName = filenamify(mindMapPreset?.title || 'preset');
    exportFromJSON({
      data: cleanupMindMapPreset({ mindMapPreset }),
      fileName,
