...
import { useMindMapConversations, useMindMapConversation } from '~/hooks';
...
export default function MindMapConversation({ conversation, retainView }) {
  const { showToast } = useToastContext();
  const [currentConversation, setCurrentConversation] = useRecoilState(store.mindMapConversation);
  ...
  const clickHandler = async () => {
    ...
    if (conversation?.endpoint === 'gptPlugins') {
      ...
      switchToMindMapConversation({ ...conversation, tools: lastSelectedTools });
    } else {
      switchToMindMapConversation(conversation);
    }
  };
