...
import { useMindMapConversations, useNavigateToMindMapConvo } from '~/hooks';
...
export default function MindMapConversation({ conversation, retainView, toggleNav, i }) {
  const { conversationId: currentConvoId } = useParams();
  const activeConvos = useRecoilValue(store.allMindMapConversationsSelector);
  ...
  const clickHandler = async () => {
    ...
    if (conversation?.endpoint === 'gptPlugins') {
      ...
      navigateToMindMapConvo({ ...conversation, tools: lastSelectedTools });
    } else {
      navigateToMindMapConvo(conversation);
    }
  };
  ...
  const onRename = (e: MouseEvent<HTMLButtonElement> | FocusEvent<HTMLInputElement> | KeyEvent) => {
    ...
    updateConvoMutation.mutate(
      ...
      {
        onSuccess: () => refreshMindMapConversations(),
        onError: () => {
          setTitleInput(title);
          showToast({
