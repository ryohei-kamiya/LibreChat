...
export default function Messages({ isSearchView = false }) {
  ...
  const mindMapMessagesTree = useRecoilValue(store.mindMapMessagesTree);
  const showPopover = useRecoilValue(store.showPopover);
  const setAbortScroll = useSetRecoilState(store.abortScrollFamily(0));
  const searchResultMessagesTree = useRecoilValue(store.searchResultMessagesTree);

  const _messagesTree = isSearchView ? searchResultMessagesTree : mindMapMessagesTree;
