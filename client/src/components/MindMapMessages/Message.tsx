...
import { useMindMapMessageHandler, useMindMapConversation } from '~/hooks';
...
export default function Message(props: TMessageProps) {
  ...
  const setLatestMindMapMessage = useSetRecoilState(
    store.latestMindMapMessageFamily(props.nodeId ?? store.initNodeId),
  );
  const [abortScroll, setAbortScroll] = useRecoilState(store.abortScrollFamily(props.id ?? 0));
  const { isSubmitting, ask, regenerate, handleContinue } = useMindMapMessageHandler(
    props.id,
    props.paramId,
    props.nodeId,
  );
  const { switchToMindMapConversation } = useMindMapConversation();
  ...
  }, [isLast, message, setLatestMindMapMessage]);
  ...
  const clickSearchResult = async () => {
    ...
    if (response.data) {
      switchToMindMapConversation(response.data);
    }
  };
