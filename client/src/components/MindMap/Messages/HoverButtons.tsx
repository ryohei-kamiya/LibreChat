...
import { useMindMapGenerationsByLatest, useLocalize } from '~/hooks';
...
type THoverButtons = {
  ...
  mindMapConversation: TConversation | null;
  isSubmitting: boolean;
  mindMapMessage: TMessage;
  regenerate: () => void;
...
export default function HoverButtons({
  ...
  const { endpoint: _endpoint, endpointType } = mindMapConversation ?? {};
  const endpoint = endpointType ?? _endpoint;
  const [isCopied, setIsCopied] = useState(false);
  const { hideEditButton, regenerateEnabled, continueSupported } = useMindMapGenerationsByLatest({
    isEditing,
    isSubmitting,
    mindMapMessage,
    endpoint: endpoint ?? '',
  ...
  if (!mindMapConversation) {
    return null;
  }

  const { isCreatedByUser } = mindMapMessage;
