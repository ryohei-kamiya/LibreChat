    ...
    if (!newEndpoint) {
      ...
      if (modularChat && conversationId && conversationId !== 'new') {
        ...
        const currentConvo = getDefaultConversation({
          ...
          conversation: { ...(mindMapConversation ?? {}), endpointType: template.endpointType },
          preset: template,
        });

        /* We don't reset the latest message, only when changing settings mid-converstion */
        newMindMapConversation({ template: currentConvo, keepLatestMessage: true });
        return;
      }
      newMindMapConversation({ template });
    }
  };
