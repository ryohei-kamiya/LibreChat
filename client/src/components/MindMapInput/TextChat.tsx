...
import { useMindMapMessageHandler, ThemeContext } from '~/hooks';
...
export default function TextChat({
  ...
  const { ask, isSubmitting, handleStopGenerating, latestMindMapMessage, endpointsConfig } =
    useMindMapMessageHandler(id, paramId, nodeId);
  const mindMapConversation = useRecoilValue(store.mindMapConversation);
  ...
  const updateHasText = useCallback(
    (text: string) => {
      setHasText(!!text.trim() || !!latestMindMapMessage?.error);
    },
    [setHasText, latestMindMapMessage],
  );

  useEffect(() => {
    updateHasText(text);
  }, [text, latestMindMapMessage, updateHasText]);
  ...
  return (
    <>
      ...
      >
        <OptionsBar id={id} paramId={paramId} nodeId={nodeId} />
            ...
            <div className="relative flex h-full flex-1 md:flex-col">
              ...
              >
                ...
                <SubmitButton
                  ...
                  userProvidesKey={
                    mindMapConversation?.endpoint
                      ? endpointsConfig?.[mindMapConversation.endpoint]?.userProvide
                      : undefined
                  }
                  hasText={hasText}
                />
