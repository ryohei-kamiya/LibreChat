...
import FileUpload from '~/components/MindMapInput/EndpointMenu/FileUpload';
import { useLocalize, useMindMapMultipleKeys } from '~/hooks';
...
const GoogleConfig = ({ userKey, setUserKey }: Pick<TConfigProps, 'userKey' | 'setUserKey'>) => {
  ...
  const { getMultiKey, setMultiKey } = useMindMapMultipleKeys(setUserKey);
