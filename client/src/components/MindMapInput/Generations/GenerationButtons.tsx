...
import { useMindMapMessageHandler, useMediaQuery, useGenerations } from '~/hooks';
...
export default function GenerationButtons({
  ...
  const { continueSupported, regenerateEnabled } = useGenerations({
    endpoint,
    message: latestMindMapMessage as TMessage,
    isSubmitting,
  });
  ...
  } else if (mindMapMessages && mindMapMessages.length > 0 && regenerateEnabled) {
