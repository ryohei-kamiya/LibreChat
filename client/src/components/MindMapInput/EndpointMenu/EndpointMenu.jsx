...
import { cn, cleanupMindMapPreset, mapEndpoints } from '~/utils';
import {
  useLocalize,
  useLocalStorage,
  useMindMapConversation,
  useDefaultMindMapConvo,
} from '~/hooks';
...
export default function NewConversationMenu() {
  ...
  const [mindMapConversation, setMindMapConversation] =
    useRecoilState(store.mindMapConversation) ?? {};
  ...
  const onFileSelected = (jsonData) => {
    const jsonPreset = { ...cleanupMindMapPreset({ preset: jsonData }), presetId: null };
  ...
  useEffect(() => {
    if (endpoint && endpoint !== 'bingAI') {
      const lastModelUpdate = { ...lastModel, [endpoint]: mindMapConversation?.model };
      if (endpoint === 'gptPlugins') {
        lastModelUpdate.secondaryModel = mindMapConversation.agentOptions.model;
      }
      setLastModel(lastModelUpdate);
    } else if (endpoint === 'bingAI') {
      const { jailbreak, toneStyle } = mindMapConversation;
      setLastBingSettings({ ...lastBingSettings, jailbreak, toneStyle });
    }

    setLastMindMapConvo(mindMapConversation);
  }, [mindMapConversation]);
  ...
  const onSelectEndpoint = (newEndpoint) => {
    ...
    if (!newEndpoint) {
      ...
      newMindMapConversation(null, { endpoint: newEndpoint });
    }
  };
  ...
  const onSelectPreset = (newPreset) => {
    ...
    if (
      ...
      setMindMapConversation(currentConvo);
      setMindMapMessages(mindMapMessages);
      return;
    }

    newMindMapConversation({}, newPreset);
  };
